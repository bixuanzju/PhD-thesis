...
We naturally extend the subtyping rules of \namee in \cref{fig:subtype_decl}
with only one rule \rref*{S-forall}:
\[
  \drule{S-forall}
\]
which specifies the subtyping relation between two universal quantifiers. In
\rref{S-forall}, a universal quantifier is covariant in its body, and
