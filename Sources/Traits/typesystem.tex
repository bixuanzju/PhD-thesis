...
\Cref{fig:trait:typesystem} shows the most relevant subtyping and well-formedness
rules for \sedel. Omitted rules are standard and can be found in the appendix.
The subtyping rule for trait types (\rref{TS-trait}) resembles the one for function
...
\begin{figure}[t]
  \centering
\begin{small}
  \drules[SD]{$[[ SD |- AA ** BB ]]$}{Disjointness}{trait, traitArrOne, traitArrTwo, ax}
  \drules[SDax]{$[[ AA **a BB ]]$}{Disjointness axiom}{intTrait, traitForall, traitRec}
\end{small}
...
\Cref{fig:disjoint} shows the disjointness rules for traits. The disjointness checking is the underlying
...
\begin{figure}[t]
  \centering
  \begin{small}
  \drules[ST]{$[[ SD ; SG  |- E => AA ~~> ee]]$}{Infer}{trait,traitSuper,new,forward}
  \end{small}
...
% One representative of inference rules is
% \begin{mathpar}
%  \drule{st-merge}
% \end{mathpar}
...
(\rref{ST-traitSuper}) of \rref{Inf-trait} where it implicitly assumes a
...
\lstinputlisting[linerange=76-78]{./examples/overview2.sl}% APPLY:linerange=TRANS
...
\lstinputlisting[linerange=86-86]{./examples/overview2.sl}% APPLY:linerange=NEW
...
\begin{proof}
  ...
  terms in the premises. Then by the coherence property of \fnamee, we conclude
