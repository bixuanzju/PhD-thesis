...
\sedel supports a simple, yet expressive form of traits~\cite{scharli2003traits}.
...
\lstinputlisting[linerange=82-82]{./examples/overview2.sl}% APPLY:linerange=EDITOR_INST
...
\lstinputlisting[linerange=107-110]{./examples/overview2.sl}% APPLY:linerange=HELP
...
In \sedel, self-types of traits are known as trait \emph{requirements}.
...
generators of instances, or as units of code reuse. Another important reason why our
...
% and \lstinline{help}.
% \lstinputlisting[linerange=93-96]{./examples/overview2.sl}% APPLY:linerange=HELP2
...
Let us go back to our very first trait \lstinline{editor} in \cref{sec:trait:overview}. Note how in
...
\lstinline{version}. In \sedel there are no abstract definitions (methods or
...
\lstinputlisting[linerange=93-96]{./examples/overview2.sl}% APPLY:linerange=HELP2
\sedel uses a syntax where the self parameter
...
\lstinputlisting[linerange=114-118]{./examples/overview2.sl}% APPLY:linerange=MODAL2
...
\lstinputlisting[linerange=128-132]{./examples/overview2.sl}% APPLY:linerange=MODAL_CONFLICT
...
is selected:
\lstinputlisting[linerange=139-145]{./examples/overview2.sl}% APPLY:linerange=MODAL_OK
Now the above code type checks. We can also select \lstinline{on_key} in the \lstinline{spell} trait as easily:
\lstinputlisting[linerange=150-155]{./examples/overview2.sl}% APPLY:linerange=MODAL_OK2
...
\lstinputlisting[linerange=159-164]{./examples/overview2.sl}% APPLY:linerange=MODAL_OK3
...
% \paragraph{The Keyword Override}

% Since it is very common that we want to exclude some methods from the
% inherited traits, and redefine them in the body, \sedel provides the keyword
...
% \paragraph{The Keyword Super}

% What if we want to invoke the overridden method in the overriding method. As
% with convectional OO languages, \sedel also provides the keyword
...
\lstinputlisting[linerange=169-177]{./examples/overview2.sl}% APPLY:linerange=MODAL_WIRE
...
small test:
\lstinputlisting[linerange=181-183]{./examples/overview2.sl}% APPLY:linerange=MODAL_USE
