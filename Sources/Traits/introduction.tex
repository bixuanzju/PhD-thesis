...
Sch\"arli et al.~\citep{scharli2003traits} make a good case for the advantages of the trait
...
%% \footnote{\sedel stands for \textbf{S}afe and \textbf{E}xpressive \textbf{DEL}egation}
...
denotational semantics for inheritance~\citep{cook1989denotational}, we show how to design a source
language that can be elaborated into Alpuim et al.'s
\fname~\citep{alpuimdisjoint}, a polymorphic calculus with records supporting disjoint polymorphism.
\sedel's elaboration into \fname is proved to be both type-safe and
coherent. Coherence ensures that the semantics of
\sedel is unambiguous. In particular this property is useful to ensure that
...
Object Algebras~\citep{oliveira2012extensibility} and \textsc{Visitor}s.
In particular we show how \sedel can easily compose multiple object
...
or Scala~\citep{oliveira2013feature, rendel14attributes}. The previous state-of-the-art implementations for such
...
% famous paper~\citep{cook1989inheritance} on ``\emph{Inheritance is not Subtyping}'' that the issues were
...
% languages~\citep{america1991designing,graver1989type,chambers1992object,bruce1995polytoil}).
...
%   inheritance})~\citep{ungar1988self,chambers1992object}. Finally, we present
% \sedel\footnote{\sedel stands for: {\bf S}afe and {\bf E}xpressive {\bf
...
%   intersection types}~\citep{oliveira2016disjoint} and \emph{disjoint
%   polymorphism}~\citep{alpuimdisjoint}. The choice of structural typing is due to
...
% as Extensible Visitors~\citep{togersen:2004,oliveira09modular} or Object Algebras
...
% the covariant model, and this is precisely what \sedel does. For the second
...
% other forms of meta-programming. Better language support is desirable. \sedel
% does this by embracing delegation, and having a powerful polymorphic type system
% with intersection types and disjoint polymorphism. \sedel's type system is
...
% traits~\citep{scharli2003traits}, but based on dynamic inheritance.
% \end{comment}
...
%   types}~\citep{coppo1981functional,pottinger1980type} with a \emph{merge
%   construct}~\citep{dunfield2014elaborating}. In \sedel subtyping is modelled with
% disjoint intersection types, and inheritance is modelled with the merge
% construct. \sedel inherits the key properties of these calculi, which are
...
\item \textbf{Elaboration of first-class traits into disjoint intersection types/polymorphism:}
  We show how the semantics of
  \sedel can be defined by elaboration into Alpuim et al.'s
  \fname~\citep{alpuimdisjoint}. The elaboration is inspired by the
  work of Cook and Palsberg~\citep{cook1989denotational} to model inheritance.
...
% operator for inheritance~\citep{}. This requires a very dynamic form of
