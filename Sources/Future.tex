...
as it is. An alternative to the matching judgment is the \textit{application judgment}
...
interested in adding \textit{iso-recursive types}, where a recursive type $[[mu XX . A]]$ and its one-step unfolding are transformed back and forth by a pair
...
\begin{mathpar}
  ...
  \drule{Dax-recRVar}
\end{mathpar}
An important observation is that any two distinct type variables are \textit{not}
