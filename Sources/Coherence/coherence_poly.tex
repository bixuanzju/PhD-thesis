...
In this chapter, we extend the canonicity logical relation introduced in
...
show that type system only produces \textit{well-formed} types. Recall the
...
theorem: subtyping of \textit{well-formed} types produces \textit{unique} coercions.
...
\begin{figure}
  ...
  \end{tabular}
  \end{small}
  \caption{The canonicity logical relation for \fnamee}
...
canonicity logical relation in \cref{fig:logical} with a new clause (highlighted) for
...
\begin{theorem}[Coherence of \fnamee] \label{thm:coherence:fi}
  We have that
  \begin{itemize}
