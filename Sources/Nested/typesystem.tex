...
Henglein~\cite{Henglein_1994}. A full technical comparison with \oname can be found in \cref{sec:comparision}.
...
It can be shown that the other direction ($[[A1 -> A2 & A3 <: (A1 -> A2) & (A1 -> A3)]]$)
...
The bidirectional type system for \name is shown in \cref{fig:type_system}. Again we ignore the \hll{highlighted}
parts for now.
...
Space-efficient coercions can be obtained by following the approach such as \cite{Siek_2015, herman2010space}.
...
Below we identify major differences from \oname, which, when
...
\oname subtyping relation. The subtyping relation of \oname employ an
