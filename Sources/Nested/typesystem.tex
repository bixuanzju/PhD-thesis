...
\oname~\citep[Figure 10]{oliveira2016disjoint}, where the latter does not have
\rref{D-topL,D-topR}.
...
record labels, i.e., labels are erased during elaboration. (This is different from
the original publication~\citep{bi_et_al:LIPIcs:2018:9227} where we keep labels in the target.)
% It is equally fine to % keep labels in the target.
The notation $| \cdot |$ is also overloaded for
context translation from $[[GG]]$ to $[[gg]]$.
...
coercion parts in \rref{Si-andL,Si-andR}). For \namee, as with ordinary types, we
...
that is still coherent. As a consequence, \namee is already more expressive than
\oname even without adding BCD subtyping: an expression such as $1 : [[nat &
nat]]$ is accepted in \namee but rejected in \oname. This simplification is
based on an important observation: incoherence can only originate in merges.
Therefore disjointness checking is only necessary in \rref{T-merge}.
