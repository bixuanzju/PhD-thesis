...
\begin{enumerate}
\item The top type $[[Top]]$ can be regarded as the nullary intersection. It is
...
follows from \rref{S-interL,S-interior} and the subsumption rule; so we need not
...
\citet{reynolds1997design} demonstrated the power of the merge operator by
developing an encoding of records by using intersection types; similar ideas also appear in \citet{Castagna_1992}. The idea
is to add single-field records as a primitive notion, with the introduction form
$[[ { l = e } ]]$ of type $[[ {l : A} ]]$ and the usual elimination form. Thus
instead of
\[
  [[ {  l1 = e1, ... , ln = en   }       ]]
\]
...
bidirectional type-checking, \citeauthor{reynolds1988preliminary} formalized the
\oname calculus, which ensures that there is at most one elaboration derivation
...
expressiveness power to calculi with disjoint intersection types.
