...
\begin{mathpar}
  ...
  \inferrule*[right=S-interR]{ }{ [[A & B <: B]] } \and
...
$[[B]]$:
\[
    \inferrule*[right=T-interI]{ [[ee : A1]] \\  [[ee : A2]]   }{   [[ee : A1 & A2]]   }
\]
...
\begin{mathpar}
    \inferrule*[right=T-mergeL]{ [[  ee1 : A  ]]   }{ [[  ee1 ,, ee2 : A   ]] } \and
    \inferrule*[right=T-mergeR]{ [[  ee2 : A  ]]   }{ [[  ee1 ,, ee2 : A   ]] }
\end{mathpar}
...
\begin{figure}
  \centering
  \begin{small}
  \drules[s]{$[[A <: B ~~> e]]$}{Subtyping}{int, top, arr, and, andl, andr}
  \drules[wf]{$[[GG |- A]]$}{Well-formedness of types}{int, top, and, arr}
  \drules[T]{$[[GG  |- ee => A ~~> e]]$}{Inference}{top, lit, var, app, anno, merge}
  \drules[T]{$[[GG  |- ee <= A ~~> e]]$}{Checking}{lam, Sub}
  \end{small}
...
unique coercions, they also employ the notion of \textit{ordinary types} ---
those which are not intersection types --- and use the judgment $[[ A ord ]]$ in
\rref{s-andl, s-andr}. Ordinary types and disjointness are sufficient to ensure
a coherent semantics of a type system without $[[Top]]$.
...
of the merge is chosen. In other words, $[[Top]]$ is always disjoint to every
other type. This restriction was later lifted by \citet{alpuimdisjoint} in the
\fname calculus by a set of inference rules, but whether a specification of
